...
def Ph2_ACFRootExtractor(infile,Scan_n,type):
    print("starting Ph2_ACFRootExtractor")
    print("Detector/Board_0/OpticalGroup_0/Hybrid_"+H_ID+"/Chip_"+str(int(C_ID))+"/D_B(0)_O(0)_H("+H_ID+")_"+Scan_n+"_Chip("+str(int(C_ID))+")")
    ...
    print("D_B(0)_O(0)_H("+H_ID+")_"+Scan_n+"_Chip("+str(int(C_ID))+")")
...
def Plots(ToTMap, NoiseMap, Noise_L, ThrMap, Thr_L, Data, Data_L, Missing_mat, Missing, Missing_strange, Perc_missing, Perc_missing_strange, Disabled, ToTMapX, FitErrors):
    ...
    fig1.savefig(Path+Module+'/'+Voltage_1+'V_Noise_Map.png', format='png', dpi=300)
    ...
    fig2.savefig(Path+Module+'/'+Voltage_1+'V_Noise_Hist.png', format='png', dpi=300)
    ...
    fig3.savefig(Path+Module+'/'+Voltage_1+'V_Threshold_Map.png', format='png', dpi=300)
    ...
    fig7.savefig(Path+Module+'/'+Voltage_1+'V_ToT_Map.png', format='png', dpi=300)
    ...
    ax.set_aspect(1)
    bar2=plt.colorbar(imgplot, orientation='horizontal', extend='max', label='ToT')
    fig10.savefig(Path+Module+'/'+Voltage_1+'V_ToT_Map_XRay.png', format='png', dpi=300)
    ...
    fig4.savefig(Path+Module+'/'+Voltage_1+'V_Threshold_Hist.png', format='png', dpi=300)
    ...
    ax.set_ylabel('entries')
    ax.legend(prop={'size': 14}, loc='upper right')
    fig5.savefig(Path+Module+'/'+Voltage_1+'V_Hist_Thr_'+str(Thr)+'_'+str(Thr_strange)+'.png', format='png', dpi=300)
    ...
    ax.set_aspect(1)
    bar2=plt.colorbar(imgplot, orientation='horizontal', extend='max', label='Hits')
    fig9.savefig(Path+Module+'/'+Voltage_1+'_XRay_Hits_Map_zoom.png', format='png', dpi=300)
    ...
    #fig6.savefig(Path+Module+'/'+'chip_'+str(int(C_ID))+'_Missing_Bumps_Thr_'+str(Thr)+'_'+str(Thr_strange)+'.png')
    ...
    output_file = ROOT.TFile('/home/kucms/TrackerDAQ/modules/Ph2_ACF/DAQSettings_v1/SD25_analyzed.root', 'RECREATE')
    missing_hist.Write()
    output_file.Close()
